...
class UserNetworkManager(models.Manager):
    def get_followers(self, user):
        queryset = super(UserNetworkManager, self).get_query_set().filter(user=user, status=1)
        return [ x.follower for x in queryset ]
    
    def get_following_users(self, user):
        queryset = super(UserNetworkManager, self).get_query_set().filter(follower=user, status=1)
    ...
    def unfollow_user(self, user, following):
        try:
            user_network = self.get_query_set().get(user=following, follower=user, status=1)
        except Exception, e:
            print e
        else:
            user_network.delete()
    ...
    def ban_follower(self, user, follower):
        try:
            user_network = self.get_query_set().get(user=user, follower=follower, status=1)
        except Exception, e:
            print e
        else:
...
#                pass
            ...
            subject = "%(follower)s follows you on Befree" % {'follower' : self.follower.get_full_name()}
            title = ''
            message = subject
