...
TYPE_CHOICES = (
    ...
    ('TEA', _('Buddhist Teacher')),
    ('BUD', _('Regular Buddhist')),
    ('Other',_('Other')),
)
...
class Profile(UserenaBaseProfile):
    ...
    def validate_giver(self, local_pickup, domestic_pay_shipping,domestic_free_shipping,international_free_shipping,pickup_description, paypal_email, payment_description):
        if not local_pickup and not domestic_free_shipping \
            ...
            raise ValidationError(_("Please choose at least one distribution method. "))
        if domestic_pay_shipping and domestic_free_shipping:
            raise ValidationError(_("Please choose either  Free Domestic Shipping or Receive Payment for Domestic Shipping, not both."))
        if local_pickup and not pickup_description:
            raise ValidationError(_("Since you allow user to pickup locally, please provide pickup instruction. "))
        if domestic_pay_shipping and not ( paypal_email or payment_description):
            raise ValidationError(_("Sorry receivers need to pay your for shipping. Either Paypal Email or Other Payment Methods is required for giver profile. "))
