...
class Material(models.Model):
    ...
    pic = models.ImageField(upload_to=cover_pic_name, null=True, blank=True,
                            verbose_name="Upload Cover Picture")
    pdf = models.FileField(upload_to=pdf_name, null=True, blank=True,
                           verbose_name='Upload PDF File')
    website = models.URLField(max_length=255, null=True, blank=True)
...
class GiverMaterial(models.Model):
    ...
    def __unicode__(self):
        return "%s - %s " % (
            self.giver.get_profile().get_display_name(), self.material )
