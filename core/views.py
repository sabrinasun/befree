...
from core.forms import MaterialForm, AuthorForm, PublisherForm, GiverMaterialForm, ContactForm, PayForm, \
    ShippingCostForm
...
def get_order_from_bag(cart, user):
    inventories = GiverMaterial.objects.filter(pk__in=[int(id) for id in cart]).order_by('giver__username',
                                                                                         'material__title')
    ...
    for key in orders:
        ...
        if max_per_order != 0 and free_count > max_per_order:
            warning.append(
                "You ordered %s items, which are more than a small quantity order of %s items for this giver, order will be in pending status until the giver approves it." % (
                free_count, max_per_order))

        ret.append({"giver": orders[key][0]['inventory'].giver, "order_details": orders[key], "price": price,
                    "shipping_cost": shipping_cost, "total": "%.2f" % (price + Decimal(shipping_cost)),
                    "weight": weight, "warning": warning, "shipping_cost_wave": shipping_cost_wave})
...
def contact_user(request):
    ...
    if request.method == "POST":
        ...
        if contact_form.is_valid():
            ...
            return HttpResponse(
                '<script type="text/javascript">document.write("Your message has been sent.");window.close();opener.alert("You email message has been sent.");</script>')
    ...
    if not request.user.is_authenticated():
        return HttpResponse(
            '<script type="text/javascript">alert("Please login to your account before you access the contact link.");document.location="/";</script>')
...
def pay_giver(request):
    ...
    if request.method == "POST":
        ...
        if pay_form.is_valid():
            ...
            return HttpResponse(
                '<script type="text/javascript">opener.location="/account/orders/reading/";window.close();</script>')
