...
def send_email(template, context, title, to_address):
    ...
    send_mail(title, content, settings.DEFAULT_FROM_EMAIL, [to_address],
              fail_silently=False)
...
def index(request):
    ...
    if location != 'all' and location != '':
        # users = [p.user for p in Profile.objects.all().filter( Q(
...
The increments for shipping cost are 43 cents per pound, plus packaing cost
10 cents, all the way up to $32.32 for the 70 lbs max.
"""
...
def get_order_from_bag(cart, user):
    inventories = GiverMaterial.objects.filter(
        pk__in=[int(id) for id in cart]).order_by('giver__username',
                                                  'material__title')
    ...
    for key in orders:
        ...
        if shipping_cost == -1:
            warning.append(
                "We can't determine shipping cost. Wait for giver's advice "
                "after placing order. ")
        ...
        ret.append({"giver": orders[key][0]['inventory'].giver,
                    ...
                    "total": "%.2f" % (price + Decimal(shipping_cost)),
                    "weight": weight, "warning": warning,
                    "shipping_cost_wave": shipping_cost_wave})
...
def view_bag(request):
    ...
    if not request.user.is_authenticated():
        ...
        return redirect(reverse_lazy('userena_profile_edit_from_view_bag',
    ...
    context = {"orders": orders, "action": "view_bag",
               "outofstock_id": int(outofstock_id)}
...
def contact_user(request):
    ...
    if request.method == "POST":
        ...
        if contact_form.is_valid():
            to_user = get_object_or_404(get_user_model(),
                                        id=contact_form.cleaned_data[
                                            'user_id'])
            message = contact_form.cleaned_data['message']
            context = Context(
                {'to_user': to_user.get_profile().get_display_name(),
            ...
            return HttpResponse(
                '<script type="text/javascript">document.write("Your '
                'message has been sent.");window.close();opener.alert("You '
                'email message has been sent.");</script>')
    ...
    if not request.user.is_authenticated():
        return HttpResponse(
            '<script type="text/javascript">alert("Please login to your '
            'account before you access the contact '
            'link.");document.location="/";</script>')
...
def check_out(request):
    ...
    for one_order in orders:
        for item in one_order['order_details']:
            ...
            if ( left < 0):
                return redirect(
                    '/view_bag?outofstock_id=' + str(inventory.id))
    ...
    for one_order in orders:
        with transaction.commit_on_success():
            ...
            send_email('email-reader-order-placed.txt', context, title,
                       reader.email)
...
def confirm_check_out(request):
    ...
    with transaction.commit_on_success():
        ...
        if Order.objects.filter(reader=request.user, material=material):
            order = Order.objects.filter(reader=request.user,
            material=material)[0]
            order.quantity += quantity
            order.save(update_fields=['quantity'])
        else:
            Order.objects.create(reader=request.user, material=material,
            quantity=quantity)
...
def account_summary(request):
    ...
    materials = Material.objects.filter(id__in=
                                        [g.material.pk for g in
                                         giver_materials if g.material])
    context = {
        'reading_orders_new': reading_orders.filter(ship_date=None,
                                                    pay_date=None).count(),
        'reading_orders_shipped': reading_orders.exclude(
            ship_date=None).filter(pay_date=None).count(),
        'reading_orders_delivered': reading_orders.exclude(
            pay_date=None).count(),
        'giving_orders_new': giving_orders.filter(ship_date=None,
                                                  pay_date=None).count(),
        'giving_orders_shipped': giving_orders.exclude(ship_date=None).filter(
            pay_date=None).count(),
        'giving_orders_delivered': giving_orders.exclude(
            pay_date=None).count(),
        'materials_active': giver_materials.filter(status='Active').count(),
        'materials_inactive': giver_materials.filter(
...
def account_reading_orders(request):
    ...
    if order_id:
        ...
        send_email('email-reader-order-cancelled-by-reader.txt', context,
                   title, order.reader.email)
        title = "You have cancelled order %s. " % order.pk
        send_email('email-giver-order-cancelled-by-reader.txt', context,
                   title, order.giver.email)
    ...
    ret = [{'order': order, 'detail': order_details.filter(order=order)}
           for order in orders]
...
def account_giving_orders(request):
    ...
    if order_id:
        ...
        if status == "SHIPPED":
            ...
            send_email('email-reader-order-shipped.txt', context, title,
                       order.reader.email)

        elif status == "CANCEL":
            title = "Your order %s has been cancelled. " % order.pk
            send_email('email-reader-order-cancelled-by-giver.txt', context,
                       title, order.reader.email)
            title = "You have cancelled order %s. " % order.pk
            send_email('email-giver-order-cancelled-by-giver.txt', context,
                       title, order.giver.email)

        elif status == "NEW":
            title = "Your order %s has been approved. " % order.pk
            send_email('email-reader-order-approved.txt', context, title,
                       order.reader.email)
    ...
    if request.method == "POST":
        ...
        if shipping_form.is_valid():
            order = Order.objects.get(
                id=shipping_form.cleaned_data['order_id'])
...
def account_material(request):
    if not request.user.get_profile().has_giver_profile():
        return redirect(reverse_lazy('userena_profile_edit_from_inventory',
                                     args=[request.user.username]))
                ...
                if request.method == 'POST':
                    ...
                    if form.is_valid():
                        ...
                        if new_obj:
                            return HttpResponse(
                                '<script '
                                'type="text/javascript">opener.location.reload();window.close();</script>')
                else:
                    form = form()
...
def account_add_publisher(request):
    ...
    if request.method == 'POST':
        ...
        form = PublisherForm()
    return render(request, 'account/add_publisher.html',
                  {'form': form, 'next': next})
...
def pay_giver(request):
    ...
    if request.method == "POST":
        ...
        if pay_form.is_valid():
            ...
            if pay_method == "other":
                pay_method = "'other payment method' you specified in your " \
                             "giver's profile"
            ...
            send_email('email-giver-order-paid.txt', context, title,
                       giver_email)
            send_email('email-reader-order-paid.txt', context, title,
                       request.user.email)

            return HttpResponse(
                '<script type="text/javascript">opener.location="/account'
                '/orders/reading/";window.close();</script>')
